...
class CustomPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_value_provider_argument(
            ...
            default = '{}-analytics-data-lake/Accounts/PetDeleted/Pet_Deleted_Output_testing.json'.format(env))
        parser.add_value_provider_argument(
            '--output',
            type=str,
...
def streaming_pipeline(project, region="us-central1"):
    ...
    subscription = "projects/{}/subscriptions/Pet_Deleted".format(project)
    bucket = "gs://{}-analytics-data-lake/Accounts/PetDeleted/".format(env)
    ...
    options = PipelineOptions(
        ...
        template_location = 'gs://{}-analytics-temp-files/Accounts/AM_PetDeleted_PStoGCS.py'.format(env),
    ...
    class WriteToGCS(beam.DoFn):
        def __init__(self):
            self.outdir = "gs://{}-analytics-data-lake/Accounts/PetDeleted/".format(env)
        ...
        def process(self, element):
            ...
            file_prefix = "Pet_Deleted_Output_" + timestampStr + '.json' 
            writer = FileSystems.create(self.outdir + file_prefix, 'text/plain')
            writer.write(element.encode())
            writer.close()

    subscription = "projects/{}/subscriptions/Pet_Deleted".format(project)
    topic = "projects/{}/topics/pet-deleted".format(project)
    bucket = "gs://{}-analytics-data-lake/Accounts/PetDeleted/".format(env)
