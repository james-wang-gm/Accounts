#Pub/Sub to GCS Shopify Orders Create pipeline
...
class CustomPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_value_provider_argument(
            ...
            default = '{}-analytics-data-lake/Accounts/Members_Joined_Household_Testing.json'.format(env))
        parser.add_value_provider_argument(
            '--output',
            type=str,
...
def streaming_pipeline(project, region="us-central1"):
    ...
    subscription = "projects/{}/subscriptions/Members_Joined_Household".format(project)
    bucket = 'gs://{}-analytics-data-lake/Accounts/MembersJoinedHousehold/'.format(env)
    ...
    options = PipelineOptions(
        ...
        template_location = 'gs://{}-analytics-temp-files/Accounts/AM_MembersJoinedHousehold_PStoGCS.py'.format(env),
    ...
    class WriteToGCS(beam.DoFn):
        def __init__(self):
            self.outdir = 'gs://{}-analytics-data-lake/Accounts/MembersJoinedHousehold/'.format(env)
        ...
        def process(self, element):
            ...
            file_prefix = "Members_Joined_Household_" + timestampStr + '.json' 
            writer = FileSystems.create(self.outdir + file_prefix, 'text/plain')
            writer.write(element.encode())
            writer.close()

    subscription = "projects/{}/subscriptions/Members_Joined_Household".format(project)
    topic = "projects/{}/topics/members-joined-household".format(project)
    bucket = 'gs://{}-analytics-data-lake/Accounts/MembersJoinedHousehold/'.format(env)
